...
class LoadedMotions(nn.Module):
    def __init__(
        ...
        if activations is not None:
            self.register_buffer("activations", activations, persistent=False)
        else:
...
class MotionLib(DeviceDtypeModuleMixin):
    ...
    def get_motion_activations(self, motion_ids, motion_times):
        """Get activations for given motion IDs and times.

        Args:
            motion_ids: Tensor of motion IDs [N]
            motion_times: Tensor of motion times in seconds [N]

        Returns:
            Tensor of activations [N, num_muscles] or None if activations are not available
        """
        ...
        # Calculate frame indices and blend
        frame_idx0, frame_idx1, blend = self._calc_frame_blend(
            motion_times, motion_len, num_frames, dt
        )

        # Get absolute frame indices in the concatenated activations buffer
        f0l = frame_idx0 + self.length_starts[motion_ids]
        f1l = frame_idx1 + self.length_starts[motion_ids]

        # Sample activations
        act0 = self.state.activations[f0l]  # [N, num_muscles]
        act1 = self.state.activations[f1l]  # [N, num_muscles]

        # Blend
        blend = blend.unsqueeze(-1)  # [N, 1]
        activations = (1.0 - blend) * act0 + blend * act1
    ...
    def _load_motion_file(motion_file):
        if motion_file.endswith(".npz"):
            tmp_data = np.load(motion_file, allow_pickle=True)
            if "activations" in tmp_data.keys():
                return MotionLib._convert_rollout_npz_to_skeleton_motion(tmp_data)
            else:
                return SkeletonMotion.from_file(motion_file), None
        else:
            return SkeletonMotion.from_file(motion_file), None

    @staticmethod
    def _convert_rollout_npz_to_skeleton_motion(npz_data):
        """Convert rollout npz data to SkeletonMotion.

        Args:
            npz_data: Loaded npz file containing rollout data with 'activations' field

        Returns:
            Tuple of (SkeletonMotion, activations_tensor)
        """
        # Load original motion file to get skeleton tree
        motion_file = str(npz_data["motion_file"])
        original_motion = SkeletonMotion.from_file(motion_file)

        # Extract data from npz
        body_pos = torch.tensor(npz_data["body_pos"], dtype=torch.float32)  # [T, num_bodies, 3]
        body_rot = torch.tensor(npz_data["body_rot"], dtype=torch.float32)  # [T, num_bodies, 4]

        # Use first body (root) for root translation
        root_translation = body_pos[:, 0, :]  # [T, 3]

        # Create skeleton state from global rotations and root translation
        # is_local=False because body_rot contains global rotations
        new_sk_state = SkeletonState.from_rotation_and_root_translation(
            original_motion.skeleton_tree,
            body_rot,
            root_translation,
            is_local=False,
        )

        # Get FPS from npz, fallback to original motion FPS if not available
        if "motion_fps" in npz_data.keys():
            motion_fps = float(npz_data["motion_fps"])
        else:
            motion_fps = original_motion.fps

        new_motion = SkeletonMotion.from_skeleton_state(new_sk_state, fps=motion_fps)

        # Extract and return activations
        activations = torch.from_numpy(npz_data["activations"]).float()  # [T, num_muscles]

        return new_motion, activations
    ...
    @staticmethod
    def _slice_activations(activations, motion_timings, fps):
        """Slice activations tensor according to motion timings.

        Args:
            activations: [T, num_muscles] tensor of activations
            motion_timings: [start_time, end_time] in seconds
            fps: frames per second of the motion

        Returns:
            [T_sliced, num_muscles] tensor of sliced activations
        """
        start, end = motion_timings
        start_frame = round(start * fps)
        if end == -1:
            end_frame = activations.shape[0]
        else:
            end_frame = int(end * fps)

        return activations[start_frame:end_frame].clone()
    ...
    def _load_motions(self, motion_file, target_frame_rate):
        ...
        has_text_embeddings = []
        motion_fpses = []
        motion_activations = []  # List to collect activations
        ...
        for f in range(num_sub_motions):
            ...
            curr_motion, curr_activations = self._load_motion_file(curr_file)
            ...
            # Slice activations if available
            if curr_activations is not None:
                has_activations = True
                sub_activations = self._slice_activations(curr_activations, motion_timings[f], curr_motion.fps)
            else:
            ...
            motion_activations.append(sub_activations)
        ...
        # Concatenate all activations if available
        if has_activations:
            activations_concat = torch.cat(motion_activations, dim=0).to(device=self._device)
        else:
            activations_concat = None
