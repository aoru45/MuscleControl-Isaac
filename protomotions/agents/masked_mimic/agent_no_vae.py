...
class MaskedMimicNoVae(PPO):
    ...
    def model_step(self, batch_dict) -> Tuple[Tensor, Dict]:
        ...
        """
        activations = self.model.act(batch_dict)
        expert_tau = batch_dict["tau"]
        pred_tau = self.env.simulator.muscle_ctl._compute_torque(activations, batch_dict["JtA"], batch_dict["b"])

        bc_loss = torch.square(pred_tau - expert_tau).mean()

        extra_loss, extra_log_dict = self.calculate_extra_loss(batch_dict, activations)
    ...
    def get_JtA3(self, JtA):
        ...
        return jta_top3
    def update_obs(self, obs, last_action=None):
        mus_feats = self.env.simulator.update_muscle_features()
        JtA = mus_feats["JtA"]
        JtA3 = self.get_JtA3(JtA)
        b = mus_feats["b"]
        if last_action is None:
            expert_action = self.expert_model.act(obs) if self.expert_model is not None else action
        else:
            expert_action = last_action
        common_dof_state = self.env.simulator.get_dof_state()
        ...
        self.env.simulator.set_torque_limits(tau_min, tau_max)
        tau = self.env.simulator._compute_tau_from_actions(expert_action, q, qd)
        obs["JtA3"] = JtA3
        obs["b"] = b
        obs["tau"] = tau
        return {
    ...
    def fit(self):
        ...
        self.experience_buffer.register_key("JtA3", shape=(150,))
        self.experience_buffer.register_key("b", shape=(50,))
        self.experience_buffer.register_key("tau", shape=(50,))
        ...
        while self.current_epoch < self.config.max_epochs:
            ...
            with torch.no_grad():
                ...
                for step in step_iterator:
                    ...
                    item_to_update, expert_action = self.update_obs(obs, last_action)
                    if (step + 1) % 4 == 0:
                        last_action = None
                    else:
                        last_action = expert_action
                    for k, v in item_to_update.items():
                        self.experience_buffer.update_data(k, step, v)
                    activation = self.model.act(obs)
                    ...
                    if torch.isnan(activation).any():
                        raise ValueError(f"NaN in act: {activation}")

                    # Step env
                    # 这里走的是activation控制
                    next_obs, rewards, dones, terminated, extras = self.env_step(activation)
        ...
        for outer_iter in outer_iter_iterator:
            ...
            for eval_episode in range(self.config.eval_num_episodes):
                ...
                for step in range(max_len):
                    _, expert_action = self.update_obs(obs, last_action)
                    if (step + 1) % 4 == 0:
                        last_action = None
                    else:
                        last_action = expert_action
                    activation = self.model.act(obs)
                    obs, rewards, dones, terminated, extras = self.env_step(activation)
    ...
    def evaluate_policy(self):
        ...
        while self.config.max_eval_steps is None or step < self.config.max_eval_steps:
            ...
            if (step + 1) % 4 == 0:
                ...
                last_action = expert_action
            activation = self.model.act(obs)
            self.env.simulator._last_activations = torch.clamp(activation, 0.0, 1.0)
...
# 模型侧 student输入 (JtA, b, tau) -> 输出 (activation)  ok
# 训练损失  mse(teacher_tau, student_tau) ok
# rollout ACT ok