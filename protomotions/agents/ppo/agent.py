...
class PPO:
            ...
            with torch.no_grad():
                ...
                if self.config.normalize_advantage:
                    if self.fabric.world_size > 1:
                        # Sync advantages across ranks for global normalization
                        gathered_adv = self.fabric.all_gather(advantages)
                        if isinstance(gathered_adv, list):
                            gathered_adv = torch.cat(gathered_adv)
                        else:
                            gathered_adv = gathered_adv.flatten()
                        
                        adv_mean = gathered_adv.mean()
                        adv_std = gathered_adv.std()
    ...
    def optimize_model(self) -> Dict:
        ...
        # Synchronize training metrics across all GPUs before averaging
        if self.fabric.world_size > 1:
    ...
    def evaluate_policy(self):
        ...
        while self.config.max_eval_steps is None or step < self.config.max_eval_steps:
            ...
            # Get motion IDs and times from motion manager
            motion_ids = self.env.motion_manager.motion_ids
            motion_times = self.env.motion_manager.motion_times

            # Get activations for current motion state
            activations = self.motion_lib.get_motion_activations(motion_ids, motion_times)
            if activations is not None:
                self.env.simulator._last_activations = torch.clamp(activations, 0.0, 1.0)
    ...
    # Helper Functions
    # -----------------------------
    def _sync_training_metrics(self, training_log_dict: Dict):
        """
        Synchronize training metrics across all GPUs.
        Ensures that metrics are computed from the same global statistics,
        which is especially important for normalizer statistics to align gradients.
        """
        for key in list(training_log_dict.keys()):
            if type(training_log_dict[key][0]) is int:
                metric_sum = torch.tensor(training_log_dict[key][0], dtype=torch.float32, device=self.device)
            else:
                metric_sum = training_log_dict[key][0].detach().clone()
            metric_count = torch.tensor(training_log_dict[key][1], dtype=torch.float32, device=self.device)

            # Gather metrics from all ranks
            gathered_sums = self.fabric.all_gather(metric_sum)
            gathered_counts = self.fabric.all_gather(metric_count)

            # Compute global average
            total_sum = gathered_sums.sum().item()
            total_count = gathered_counts.sum().item()

            # Update the metric in the dict
            training_log_dict[key] = [total_sum, total_count]
    def eval(self):
        self.model.eval()
    ...
    def process_dataset(self, dataset):
        if self.config.normalize_values:
            # Gather values and returns from all ranks to ensure global normalization stats
            if self.fabric.world_size > 1:
                gathered_values = self.fabric.all_gather(dataset["values"])
                # Handle both packed (list of tensors) and unpacked (tensor) cases from all_gather
                if isinstance(gathered_values, list):
                    gathered_values = torch.cat(gathered_values)
                else:
                    gathered_values = gathered_values.flatten()
                
                gathered_returns = self.fabric.all_gather(dataset["returns"])
                if isinstance(gathered_returns, list):
                    gathered_returns = torch.cat(gathered_returns)
                else:
                    gathered_returns = gathered_returns.flatten()
                
                self.running_val_norm.update(gathered_values)
                self.running_val_norm.update(gathered_returns)
            else:
