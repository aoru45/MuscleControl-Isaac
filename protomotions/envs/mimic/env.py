...
class Mimic(BaseEnv):
    def __init__(self, config, device: torch.device, *args, **kwargs):
        ...
        left_foot_ids = [self.simulator.robot_config.body_names.index(foot_name) for foot_name in left_foot_names]
        right_foot_ids = [self.simulator.robot_config.body_names.index(foot_name) for foot_name in right_foot_names]
        self.feet_indices = left_foot_ids + right_foot_ids
    ...
    def create_visualization_markers(self):
        ...
        if not self.config.get("show_ref_motion_markers", True):
    ...
    def _compute_feet_ori_reward(self) -> Tensor:
        """
        Compute feet orientation reward (ASAP paper).
        Penalizes when feet are tilted (not flat on ground).
        Returns reward tensor [num_envs].
        """
        current_state = self.simulator.get_bodies_state()
        gr = current_state.rigid_body_rot

        gravity_vec = torch.tensor([0.0, 0.0, -1.0], device=self.device).reshape(1, 1,3).expand(self.num_envs, 3, -1)

        contact_forces = self.simulator.get_bodies_contact_buf()[:, self.feet_indices, :]  # [num_envs, 6, 3]
        contact_force_norm = torch.norm(contact_forces, dim=-1)  # [num_envs, 6]
        contact_mask = (contact_force_norm > 1.0).float()  # [num_envs, 6]

        # Get left and right feet quaternions
        left_quat = gr[:, self.feet_indices[:3]]  # [num_envs, 3, 4]
        right_quat = gr[:, self.feet_indices[3:]]  # [num_envs, 3, 4]

        # Rotate gravity vector to feet local frame (inverse rotation)
        left_gravity = rotations.quat_rotate_inverse(left_quat.reshape(-1, 4), gravity_vec.reshape(-1, 3), w_last=True).reshape(-1, 3, 3)  # [num_envs, 3, 3]
        right_gravity = rotations.quat_rotate_inverse(right_quat.reshape(-1, 4), gravity_vec.reshape(-1,3), w_last=True).reshape(-1, 3, 3)  # [num_envs, 3, 3]

        left_penalty = torch.sum(torch.square(left_gravity[..., :2]), dim=-1) ** 0.5  # [num_envs,3]
        right_penalty = torch.sum(torch.square(right_gravity[..., :2]), dim=-1) ** 0.5  # [num_envs,3]

        feet_ori_penalty = left_penalty * contact_mask[:, :3] + right_penalty * contact_mask[:, 3:]  # [num_envs, 3]
        feet_ori_penalty = feet_ori_penalty.sum(dim=-1)

        coef = self.config.mimic_reward_config.component_coefficients.get("feet_ori_c", -2.0)
        feet_ori_rew = feet_ori_penalty.mul(coef).exp()

        return feet_ori_rew

    def _compute_feet_slippage_reward(self) -> Tensor:
        current_state = self.simulator.get_bodies_state()
        foot_vel = current_state.rigid_body_vel[:, self.feet_indices]  # [num_envs, 6, 3]

        # Get contact forces for feet
        contact_forces = self.simulator.get_bodies_contact_buf()[:, self.feet_indices, :]  # [num_envs, 6, 3]

        # Compute norm of contact forces (magnitude)
        contact_force_norm = torch.norm(contact_forces, dim=-1)  # [num_envs, 6]

        # Create contact mask: 1 where contact_force > 1, 0 otherwise
        contact_mask = (contact_force_norm > 1.0).float()  # [num_envs, 6]

        # Compute foot velocity norms
        foot_vel_norm = torch.norm(foot_vel, dim=-1)  # [num_envs, 6]

        # Apply penalty only when in contact
        slippage_penalty = torch.sum(foot_vel_norm * contact_mask, dim=1)  # [num_envs]

        # Convert to reward using exponential with configured coefficient
        coef = self.config.mimic_reward_config.component_coefficients.get("feet_slippage_c", -1.0)
        feet_slippage_rew = slippage_penalty.mul(coef).exp()

        return feet_slippage_rew
    ...
    def compute_reward(self):
        ...
        # ASAP-style feet rewards: feet orientation and slippage penalty
        if len(self.feet_indices) > 0:
            # Feet orientation reward - penalize when feet are not flat (tilted)
            if self.config.mimic_reward_config.component_weights.get("feet_ori_rew_w", 0) > 0:
                feet_ori_rew = self._compute_feet_ori_reward()
                rew_dict["feet_ori_rew"] = feet_ori_rew
            else:
                rew_dict["feet_ori_rew"] = torch.zeros(self.num_envs, device=self.device)

            # Slippage penalty - penalize foot sliding when in contact with ground
            if self.config.mimic_reward_config.component_weights.get("feet_slippage_rew_w", 0) > 0:
                feet_slippage_rew = self._compute_feet_slippage_reward()
                rew_dict["feet_slippage_rew"] = feet_slippage_rew
            else:
                rew_dict["feet_slippage_rew"] = torch.zeros(self.num_envs, device=self.device)
        else:
            rew_dict["feet_ori_rew"] = torch.zeros(self.num_envs, device=self.device)
            rew_dict["feet_slippage_rew"] = torch.zeros(self.num_envs, device=self.device)
        ...
        scaled_rewards = {}
        weights = self.config.mimic_reward_config.component_weights
        for k, v in rew_dict.items():
            weight_key = f"{k}_w"
            if hasattr(weights, "get"):
                weight = weights.get(weight_key, 0.0)
            else:
                weight = getattr(weights, weight_key, 0.0)
            if weight != 0.0:
                scaled_rewards[k] = v * weight
        # scaled_rewards: Dict[str, Tensor] = {
        #     k: v * getattr(self.config.mimic_reward_config.component_weights, f"{k}_w")
        #     for k, v in rew_dict.items()
        # }

        tracking_rew = torch.zeros(self.num_envs, device=self.device)
        for v in scaled_rewards.values():
            tracking_rew += v

        total_rew = tracking_rew 
        total_rew *= float(getattr(self.config.mimic_reward_config, "reward_scale", 1.0))
        reward_clip = getattr(self.config.mimic_reward_config, "reward_clip", None)
        if reward_clip is not None:
            total_rew = torch.clamp(total_rew, min=-float(reward_clip), max=float(reward_clip))
