...
from protomotions.simulator.base_simulator.config import MarkerConfig, VisualizationMarker, MarkerState, SimulatorConfig, ControlType
...
class BaseEnv:
    ...
    def step(self, actions):
        ...
        if clamp_actions is not None and self.simulator.control_type != ControlType.TORQUE:
    ...
    def post_physics_step(self):
        ...
        if not self.disable_reset:
            self.compute_reset()
            alive_rew = (self.terminate_buf == 0).float() * self.config.mimic_reward_config.positive_constant
            self.rew_buf += alive_rew
            self.log_dict["alive_rew_mean"] = alive_rew.mean()
