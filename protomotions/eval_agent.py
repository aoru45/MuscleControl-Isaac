...
def main(override_config: OmegaConf):
    ...
    # Imports moved down to avoid loading torch before Isaac Lab app launch
    ...
    if simulator == "isaaclab":
        ...
        print("DEBUG: omni.physics.tensors in modules?", 'omni.physics.tensors' in sys.modules)
        if 'omni.physics.tensors' in sys.modules:
             print("DEBUG: omni.physics.tensors file:", sys.modules['omni.physics.tensors'].__file__)

    # Import here to ensure torch is loaded after simulation_app starts
    ...
    if override_config.get("is_diffusion_forcing", False):
        if override_config.get("diffusion_forcing_type", "dual") == "dual":
            agent.evaluate_condition_policy_diffusion_forcing()
        else:
            agent.evaluate_policy_diffusion_forcing()
    else:
