    ...
    def from_dict(cls: Type[T], config_dict: Dict[str, Any]) -> T:
        ...
        for key, value in config_dict.items():
            if value is None:
                ...
                # Check if value is dict-like (including OmegaConf DictConfig)
                is_dict_like = isinstance(value, (dict, DictConfig))
                ...
                # Handle nested dataclass fields
                    ...
                    processed_dict[key] = field_type.from_dict(value) if is_dict_like else value
                # Handle Optional types (Optional[X] is Union[X, None])
                elif get_origin(field_type) is Union:
                    args = get_args(field_type)
                    # Find the non-None type in Optional
                    inner_type = next((arg for arg in args if arg is not type(None)), None)
                    if inner_type and hasattr(inner_type, 'from_dict') and is_dict_like:
                        processed_dict[key] = inner_type.from_dict(value)
                    else:
