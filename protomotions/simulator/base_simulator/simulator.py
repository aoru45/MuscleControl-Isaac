...
def soft_bound(J, b, T_common=None):
    T = T_common.view(1, -1)
    return -T, T
    J_pos = torch.clamp(J, min=0).sum(dim=2)
    J_neg = torch.clamp(J, max=0).sum(dim=2)
    tau_max = J_pos
    tau_min = J_neg
    if T_common is not None:
        T = T_common.view(1, -1)
        tau_max = torch.clamp(tau_max+b, min=-T, max=T)
        tau_min = torch.clamp(tau_min+b, min=-T, max=T)
...
class Simulator(ABC):
    ...
    def __init__(
        ...
        # cache for muscle velocity
        self._torque_min_muscle: torch.Tensor = None
        self._torque_max_muscle: torch.Tensor = None
    ...
    def step(self, common_actions: torch.Tensor, markers_state: Optional[Dict[str, MarkerState]] = None) -> None:
        ...
        if self.control_type == ControlType.PROPORTIONAL or self.control_type == ControlType.BUILT_IN_PD:
    ...
    def _create_legged_robot_tensors(self) -> None:
        p_gains: torch.Tensor = torch.zeros(self._num_dof, dtype=torch.float, device=self.device, requires_grad=False)
        d_gains: torch.Tensor = torch.zeros(self._num_dof, dtype=torch.float, device=self.device, requires_grad=False)

        if self.robot_config.init_state is not None:
    ...
    def _compute_torques(self, action: torch.Tensor) -> torch.Tensor:
        ...
        elif self.control_type == ControlType.ACTIVATION:
            feats = self.update_muscle_features()
            JtA, b = feats["JtA"], feats["b"]
            torques = self.muscle_ctl._compute_torque(action, JtA, b)
        ...
        if self.muscle_ctl is not None:
            ...
            JtA, b = feats["JtA"], feats["b"]
            J = JtA.transpose(1, 2)
            tau_min, tau_max = soft_bound(J, b, self._torque_limits_common)
            self.set_torque_limits(tau_min, tau_max)

        if self._torque_min_muscle is not None:
            torques = torch.clip(torques, self._torque_min_muscle, self._torque_max_muscle)
        else:
            torques = torch.clip(torques, -self._torque_limits_common, self._torque_limits_common)
        return torques

    def set_torque_limits(self, torque_min_muscle: torch.Tensor, torque_max_muscle: torch.Tensor) -> None:
        self._torque_min_muscle = torque_min_muscle
        self._torque_max_muscle = torque_max_muscle
    ...
    def _compute_tau_from_actions(self, actions: torch.Tensor, q: torch.Tensor, qd: torch.Tensor) -> torch.Tensor:
        ## TODO check here
        pd_tar = actions * self._common_pd_action_scale + self._common_pd_action_offset
        p = self._common_p_gains.unsqueeze(0)
        d = self._common_d_gains.unsqueeze(0)
        tau = p * (pd_tar - q) - d * qd
        if self._torque_min_muscle is not None:
            tau = torch.clip(tau, self._torque_min_muscle, self._torque_max_muscle)
        else:
            tau = torch.clip(tau, -self._torque_limits_common, self._torque_limits_common)
        return tau

    def _compute_actions_from_tau(self, tau: torch.Tensor, q: torch.Tensor, qd: torch.Tensor) -> torch.Tensor:
        p = self._common_p_gains.unsqueeze(0)
        d = self._common_d_gains.unsqueeze(0)
        pd_tar = (tau + d * qd) / p + q
        actions = (pd_tar - self._common_pd_action_offset) / self._common_pd_action_scale
        return actions
