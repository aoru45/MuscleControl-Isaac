...
class IsaacGymSimulator(Simulator):
    ...
    def __init__(
        ...
        self._viewer = None
        self._visualize_muscles = True
        # Optional muscle visualization & features for bio robot
        ...
        if not self.headless:
            ...
            self._gym.subscribe_viewer_keyboard_event(
                self._viewer, gymapi.KEY_M, "toggle_muscle_color_mode"
            )
        ...
        self._muscle_color_mode = "activation"

        # Ensure PD gains initialized for proportional control
        try:
            self.on_environment_ready()
        except Exception as e:
            print(f"on_environment_ready failed: {e}")
    ...
    def _create_envs(
        ...
        assert robot_num_bodies == self.robot_config.num_bodies, f"Number of bodies in the config {self.robot_config.num_bodies} exceeds provided robot {robot_num_bodies}"
    ...
    def _activation_color(self, a: float):
        r = 0.4+(2.0*a)
        g = 0.4
        b = 0.4
        return (r, g, b)
    ...
    def render(self) -> None:
        if not self.headless:
            ...
            for evt in self._gym.query_viewer_action_events(self._viewer):
                ...
                elif evt.action == "toggle_muscle_color_mode" and evt.value > 0:
                    self._muscle_color_mode = "random" if self._muscle_color_mode == "activation" else "activation"
            ...
            if self._enable_viewer_sync:
                ...
                if self._visualize_muscles and self.muscle_ctl is not None:
                    self._gym.clear_lines(self._viewer)
                    act = getattr(self, "_last_activations", None)
                    if act is not None:
                        for env_id in range(self.num_envs):
                            env_ptr = self._envs[env_id]
                            pts, W_list = self.muscle_ctl.forward_batched_points(self._rigid_body_pos[env_id], self._rigid_body_rot[env_id], list(self._gym.get_asset_rigid_body_names(self._humanoid_asset)))
                            for m_idx, W in enumerate(W_list):
                                if W < 2:
                                    continue
                                mus_pts = pts[0, m_idx, :W, :].cpu().numpy()
                                a = float(act[min(env_id, act.shape[0]-1), m_idx].item()) if m_idx < act.shape[1] else 0.0
                                r, g, b = self._activation_color(a)
                                for w in range(W-1):
                                    p1 = mus_pts[w]
                                    p2 = mus_pts[w+1]
                                    verts = np.empty((1, 2), dtype=gymapi.Vec3.dtype)
                                    verts[0][0] = (float(p1[0]), float(p1[1]), float(p1[2]))
                                    verts[0][1] = (float(p2[0]), float(p2[1]), float(p2[2]))
                                    colors = np.empty(1, dtype=gymapi.Vec3.dtype)
                                    colors[0] = (r, g, b)
                                    self._gym.add_lines(self._viewer, env_ptr, 1, verts, colors)
