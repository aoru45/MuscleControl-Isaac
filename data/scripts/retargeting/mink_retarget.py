...
_KEYPOINT_TO_JOINT_MAP = {
    ...
    "bio": {
        "Torso": {"name": "Torso", "weight": 1.5, "ori": False},
        "Spine": {"name": "Spine", "weight": 1.5, "ori": False},
        # "Neck": {"name": "Neck", "weight": 1.5, "ori": False},
        "Head": {"name": "Head", "weight": 3.0, "ori": False},
        "Pelvis": {"name": "Pelvis", "weight": 2.0, "ori": False},
        "L_Hip": {"name": "FemurL", "weight": 1.0, "ori": False},
        "R_Hip": {"name": "FemurR", "weight": 1.0, "ori": False},
        "L_Knee": {"name": "TibiaL", "weight": 1.0, "ori": False},
        "R_Knee": {"name": "TibiaR", "weight": 1.0, "ori": False},
        "L_Ankle": {"name": "TalusL", "weight": 3.0, "ori": False},
        "R_Ankle": {"name": "TalusR", "weight": 3.0, "ori": False},
        # "L_Toe": {"name": "FootThumbL", "weight": 3.0, "ori": True},
        # "R_Toe": {"name": "FootThumbR", "weight": 3.0, "ori": True},
        "L_Elbow": {"name": "ArmL", "weight": 1.5, "ori": False},
        "R_Elbow": {"name": "ArmR", "weight": 1.5, "ori": False},
        "L_Wrist": {"name": "HandL", "weight": 3.0, "ori": False},
        "R_Wrist": {"name": "HandR", "weight": 3.0, "ori": False},
        "L_Shoulder": {"name": "ShoulderL", "weight": 0.0, "ori": False},
        "R_Shoulder": {"name": "ShoulderR", "weight": 0.0, "ori": False},
    },
}
...
def construct_model(robot_name: str, keypoint_names: Sequence[str]):
    ...
    if robot_name == "h1":
        ...
        humanoid_mjcf = mjcf.from_path("protomotions/data/assets/mjcf/bio.xml")
    else:
...
def retarget_motion(
    ...
    # Normalize robot name like "bio_humanoid" -> "bio"
    ...
    # Choose joint name set based on motion skeleton size (SMPL: 24, SMPL-X/H: 52)
    nbodies = motion.local_rotation.shape[1]
    mujoco_joint_names = SMPL_MUJOCO_NAMES if nbodies <= 24 else SMPLH_MUJOCO_NAMES
    model = construct_model(robot_name, mujoco_joint_names)
    configuration = mink.Configuration(model)

    tasks = []

    frame_tasks = {}
    for joint_name, retarget_info in _KEYPOINT_TO_JOINT_MAP[robot_name].items():
        if robot_name == "h1":
            orientation_base_cost = 0
        else:
            orientation_base_cost = 0.05
        task = mink.FrameTask(
            frame_name=retarget_info["name"],
            frame_type="body",
            position_cost=10.0 * retarget_info["weight"],
            orientation_cost=(orientation_base_cost * retarget_info["weight"]) if retarget_info.get("ori", False) else 0.0,
            lm_damping=1.0,
        )
    ...
    with viewer_context as viewer:
        ...
        optimization_steps_per_frame = 8  # int(max(np.ceil(5.0 * 30 / fps), 1))
        rate = RateLimiter(frequency=fps * optimization_steps_per_frame)
        # Use DAQP solver by default (fast and installed in your env)
        solver = "daqp"

        t: int = int(np.ceil(-100.0 * fps / 30))
        vel = None

        # Create progress bar (disable in multi-processing / nested tqdm contexts)
        pbar = tqdm(total=timeseries_length, desc="Retargeting frames", disable=not progress)
        ...
        while (render and viewer.is_running() or not render) and t < timeseries_length:
            if not key_callback.pause:
                ...
                for i, (joint_name, retarget_info) in enumerate(
                    ...
                    body_idx = mujoco_joint_names.index(joint_name)
                    target_pos = global_translations[max(0, t), body_idx, :].copy()

                    if robot_name in _RESCALE_FACTOR:
                        target_pos *= _RESCALE_FACTOR[robot_name]
                    if robot_name in _OFFSET:
                        target_pos[2] += _OFFSET[robot_name]
                ...
                for keypoint_name, keypoint in zip(
                        mujoco_joint_names, global_translations[max(0, t)]
                    ):
    ...
    if robot_name in ["h1", "g1"]:
        return create_robot_motion(
            retargeted_poses, retargeted_trans, global_translations, fps, robot_name
        )
    else:
        # Build SkeletonMotion from MuJoCo FK for generic robots (e.g., bio)
        skeleton_tree = SkeletonTree.from_mjcf(
            f"protomotions/data/assets/mjcf/{robot_name}.xml"
        )
        # Map skeleton nodes to MuJoCo body ids
        body_ids = []
        ...
        nb = len(body_ids)
        pose_quat_global = np.zeros((T, nb, 4), dtype=np.float64)
        global_pos = np.zeros((T, nb, 3), dtype=np.float64)
        ...
        for t in range(T):
            ...
            for i, bid in enumerate(body_ids):
                if bid >= 0:
                    wxyz = data.xquat[bid]
                    pose_quat_global[t, i] = np.array([wxyz[1], wxyz[2], wxyz[3], wxyz[0]])
                    global_pos[t, i] = data.xpos[bid]

        # Adjust global heights using feet-only lowest points to avoid bias from hands/head
        src_feet_names = [name for name in ["L_Ankle", "R_Ankle", "L_Toe", "R_Toe"] if name in mujoco_joint_names]
        src_feet_idx = [mujoco_joint_names.index(n) for n in src_feet_names]

        tgt_feet_names = []
        for k in ["L_Ankle", "R_Ankle", "L_Toe", "R_Toe"]:
            if k in _KEYPOINT_TO_JOINT_MAP[robot_name]:
                tgt_feet_names.append(_KEYPOINT_TO_JOINT_MAP[robot_name][k]["name"])

        tgt_feet_idx = []
        for n in tgt_feet_names:
            try:
        ...
        if len(src_feet_idx) > 0 and any(i >= 0 for i in tgt_feet_idx):
            # (T, 1): original lowest foot height per frame
            orig_lowest_heights = global_translations[:, src_feet_idx, 2].min(axis=1, keepdims=True)
            # (T, 1): retargeted lowest foot height per frame
            valid_tgt_z = []
            for t in range(T):
                vals = [global_pos[t, i, 2] for i in range(nb) if i in tgt_feet_idx and i >= 0]
                valid_tgt_z.append(np.min(vals) if len(vals) > 0 else global_pos[t, :, 2].min())
            retarget_lowest_heights = np.array(valid_tgt_z, dtype=np.float64).reshape(T, 1)
            # (T, 1): offset to subtract from all z positions
            height_offset = retarget_lowest_heights - orig_lowest_heights
            # Broadcast (T,1) across body dimension (T, nb)
            global_pos[..., 2] -= height_offset

        new_sk_state = SkeletonState.from_rotation_and_root_translation(
            skeleton_tree,
            torch.from_numpy(pose_quat_global),
            torch.from_numpy(global_pos[:, 0]),
            is_local=False,
        )
        return SkeletonMotion.from_skeleton_state(new_sk_state, fps=fps)
...
def manually_retarget_motion(
    ...
    sk_motion = retarget_motion(new_sk_motion, robot_type, render=render, progress=True)
